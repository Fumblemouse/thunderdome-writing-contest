        ...
        while True:
            ...
            for contest in contests:
                ...
                if contest.status == contest.UNOPENED and contest.start_date < timezone.now():
                    ...
                    contest.save()
                    #set the category for subsequent Notice if appropriate
                    if contest.mode == contest.INTERNAL_JUDGE_CONTEST:
                        category = Notice.CONTEST_ANNOUNCE_INTERNAL_JUDGE
                    elif contest.mode == contest.EXTERNAL_JUDGE_CONTEST:
                        category = Notice.CONTEST_ANNOUNCE_EXTERNAL_JUDGE
                    else:
                ...
                if contest.status == contest.OPEN and contest.expiry_date < timezone.now():
                    ...
                    #set the category for subsequent Notice if appropriate
                    if contest.mode == contest.INTERNAL_JUDGE_CONTEST:
                        category = Notice.CONTEST_CLOSE_INTERNAL_JUDGE
                    elif contest.mode == contest.EXTERNAL_JUDGE_CONTEST:
                        category = Notice.CONTEST_CLOSE_EXTERNAL_JUDGE
