...
def minidome_judgement(form, category, stories_options):
    ...
    messages.success(
        request,
        "Your judgement is final. No further corresepndance may be entered into.",
    )
    return redirect("home")


def minidome_public(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            minidome_judgement(form, MiniDome.PUBLIC, stories_options)
    else: 
        min_tests = StoryStats.objects.all().aggregate(
                min_tests=Min(F("minidome_public_wins") + F("minidome_public_losses"))
            )
        ...
        stories1 = list(
            Story.objects.filter(access__gte=2)
            .annotate(
                total_logged_in_tests=F("stats__minidome_public_wins")
                + F("stats__minidome_public_losses")
            )
        ...
        if len(stories1) == 0:
            stories1 = list(
                Story.objects.filter(access__gte=2)
                .annotate(
                    total_logged_in_tests=Sum(
                        F("stats__minidome_public_wins")
                        + F("stats__minidome_public_losses")
                    )
                )
            ...
            story1_total_public_tests = (
                story1_stats.get_minidome_total_public_tests()
            )
...
def minidome_logged_in(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            minidome_judgement(form, MiniDome.LOGGED_IN, stories_options)
    else:
        min_tests = StoryStats.objects.all().aggregate(
                min_tests=Min(
                    F("minidome_logged_in_wins") + F("minidome_logged_in_losses")
                )
            )
            min_tests_value = min_tests["min_tests"]
            if min_tests_value is None:
            ...
            # number of matches
            stories1 = list(
                Story.objects.filter(access__gte=1)
                .exclude(author=request.user)
                .annotate(
                    total_logged_in_tests=F("stats__minidome_logged_in_wins")
                    + F("stats__minidome_logged_in_losses")
                )
                .filter(total_logged_in_tests__gt=min_tests_value)
            )
            ...
            if len(stories1) == 0:
                ...
                story1_total_logged_in_tests = (
                    story1_stats.get_minidome_total_logged_in_tests()
                )
                stories2 = list(
                    Story.objects.filter(access__gte=1)
                    .exclude(author=request.user)
...
def minidome(request):
    ...
    if get_expirable_var(request.session, "minidome_judged", None):
        ...
        messages.error(
            request,
            "Go home, judge, you're drunk with power. Time until soberiety: "
            + str(int(ttl))
            + " seconds",
        )
        return redirect("home")
    ...
    if logged_in:
        stories = minidome_logged_in()
    else:
        stories = minidome_public()
    ...
    expire_at = datetime.today() + timedelta(minutes=30)
    set_expirable_var(
        request.session, "minidome_stories", [story1.pk, story2.pk], expire_at
    )
