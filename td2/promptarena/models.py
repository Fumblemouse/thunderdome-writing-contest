...
class Contest(models.Model):
    ...
    judges = models.ManyToManyField(
        settings.AUTH_USER_MODEL, through="ContestJudges", blank=True
    )
    start_date = models.DateTimeField("Start Date")
    expiry_date = models.DateTimeField("Submit by Date")
    mode = models.CharField(
        choices=CATEGORIES, default=INTERNAL_JUDGE_CONTEST, max_length=2
    )
    ...
    def save(
        ...
        if (
            self.is_active()
            and self.status != Contest.JUDGEMENT
            and self.status != Contest.CLOSED
        ):
    ...
    def get_sign_up_limit_date(self):
        """Sets up 48 hours writing time by necessity

        Returns:
            date: The time when sign-ups can no longer be entered
        """
...
class InternalJudgeContest(Contest):
    ...
    def close(self):
        ...
        judges_with_entries = [[entry, []] for entry in entries]
    ...
    def close(self):
        ...
        for judge in judges:
            ...
            for entry in shuffled_entries:
                Crit.objects.create(
                    entry=Entry.objects.get(pk=entry.id), reviewer=judge
                )
        self.status = Contest.JUDGEMENT
        self.save()
...
class Brawl(Contest):
    ...
    def close(self):
        ...
        for entry in entries:
            Crit.objects.create(
                entry=Entry.objects.get(pk=entry.id), reviewer=self.creator
            )
        self.status = Contest.JUDGEMENT
        self.save()
...
class ContestJudges(models.Model):
    ...
    contest = models.ForeignKey(
        Contest, on_delete=models.CASCADE, related_name="contestjudges"
    )
...
class Entry(models.Model):
    ...
    author = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="authors"
    )
    ...
    story = models.ForeignKey(
        Story, on_delete=models.CASCADE, related_name="entries", null=True
    )
...
class Crit(models.Model):
    ...
    entry = models.ForeignKey(
        Entry, on_delete=models.SET_NULL, null=True, related_name="crits"
    )
    ...
    def __str__(self):
        if self.reviewer and self.entry:
            return (
                str(self.entry.contest.title)
                + " : "
                + str(self.reviewer.username)
                + " reviews "
                + str(self.entry.author.username)
