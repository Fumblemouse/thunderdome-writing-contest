...
def create_contest(request):
    ...
    if contest_form.is_valid():
        ...
        messages.success(
            request,
            "Your contest was submitted successfully! Hopefully it doesn't suck.",
        )
...
def edit_contest(request, contest_id=""):
    ...
    if contest.is_active():
        messages.error(
            request, "Your contest is already live.  It cannot be edited at this time."
        )
    ...
    if contest_form.is_valid():
        ...
        messages.success(
            request,
            "Your contest was updated successfully! Hopefully you haven't confused everybody.",
        )
        return redirect("view contests")
...
def add_judge(request, contest_id=""):
    ...
    if contest_context.status == Contest.CLOSED:
        messages.error(
            request, "Contest is already closed. That ship has sailed, buddy."
        )
        return redirect("view contest details", contest_id=contest_context.pk)
    judge_form = AddJudgeForm(
...
def confirm_enter_contest(
    ...
    if request.method == "POST":
        ...
        messages.success(
            request, "You are signed up for the contest: " + contest_context.title
        )
    ...
    if Entry.objects.filter(author=request.user, contest=contest_context.pk).exists():
        messages.success(
            request, "Previous sign-up located - hit it with your best shot."
        )
        return redirect("enter contest", contest_id=contest_context.pk)
    return render(
        request,
        "promptarena/confirm-enter-contest.html",
        {"contest_context": contest_context},
    )


@login_required
def enter_contest_new_story(
    request,
    contest_id,
):
    ...
    if request.method == "POST":
        if contest_context.status != Contest.OPEN:
            messages.error(
                request, "This contest is not currently open for new entries."
            )
            # return with fields filled so work isn't lost
            return render(
                request,
                "promptarena/enter-contest-new-story.html",
                {
                    "contest_context": contest_context,
        ...
        if story_form.is_valid():
            ...
            if entry_form.is_valid():
                ...
                messages.success(
                    request,
                    "Your entry was added to your stories list and submitted successfully! Hopefully it doesn't suck.",
                )
                return redirect("view contests")
...
def enter_contest_old_story(
    ...
    if request.method == "POST":
        ...
        entry_form = EnterContestOldStoryForm(
            request.POST,
            instance=entry,
            contest_expiry_date=contest_context.expiry_date,
            contest_max_wordcount=contest_context.max_wordcount,
        ...
        if entry_form.is_valid():
            ...
            messages.success(
                request,
                "Your entry was submitted successfully! Hopefully it doesn't suck.",
            )
            return redirect("view contests")
        messages.error(request, "Something went wrong")
        return render(
            request,
            "promptarena/enter-contest-old-story.html",
            {
                "contest_context": contest_context,
                "entry_form": entry_form,
            },
    ...
    story_context = Story.objects.filter(author=request.user, has_been_public=False)
...
def judge_contest(request, contest_id=0):
    ...
    if contest_context.status != Contest.JUDGEMENT:
        messages.error(
            request, "You cannot judge a contest that is not ready for judgement"
        )
        return render(request, "promptarena/view-contest-details.html", context)
    contest_context.judge()
    messages.success(request, "You have successfully judged the contest")
    context["entry_context"] = Entry.objects.filter(contest=contest_id).order_by(
        "position"
    )
...
def view_contests(request):
    ...
    ).exclude(status=Contest.CLOSED)

    old_contest_list = Contest.objects.filter(status=Contest.CLOSED)
    if request.user.is_authenticated:
        current_user_contests_not_entered_yet = Contest.objects.filter(
            entries__author=request.user, status=Contest.OPEN, entries__content=""
        )
        current_user_contests_entered = Contest.objects.filter(
            entries__author=request.user, status=Contest.OPEN
        ).exclude(entries__content="")
    else:
...
def judgemode(request, crit_id=0):
    ...
    if request.method == "POST" and crit_form.is_valid():
        ...
        if used_scores:
            messages.error(
                request,
                "You have already used that rank in this contest. Each rank given to an entry must be different.",
            )
        else:
            ...
            messages.success(
                request, "You have successfully critted a contest entry"
            )
            return redirect("judgemode")

    crit_list = Crit.objects.filter(
        reviewer=request.user, entry__contest__status=Contest.JUDGEMENT
    ).order_by("final")
    context = {"crit_list": crit_list}
    if crit:
...
def view_contest_judgement(request, contest_id=0):
    ...
    context = {
        "contest_context": contest_context,
        "entry_context": Entry.objects.filter(contest=contest_id).order_by(
            "position"
        ),
    }
