...
class BaseAppTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.user = cls.User.objects.create_user(
            username="djangotestuser", password="12345abcde"
        )
    ...
    def set_up_story_private_with_wordcount(self, wordcount=1001):
        ...
        author = self.user
        content = "".join(" word" + str(i) for i in range(wordcount))
        self.story = Story(
            title="My Story", content=content, author=author, access=Story.PRIVATE
        )
        self.story.save()
        ...
        for story_num in range(num_stories):
            ...
            self.users.append(
                self.User.objects.create_user(
                    username="djangotestuser{}".format(story_num),
                    password="{}2345abcde".format(story_num),
                )
            )
    ...
    def set_up_contest_components(self, num_entrants=5):
        ...
        for user_num in range(num_entrants):
            self.users.append(
                self.User.objects.create_user(
                    username="djangotestuser{}".format(user_num),
                    password="{}2345abcde".format(user_num),
                )
            )
            # users[-1] = last in list
            self.users[-1].save()
        ...
        for story in self.stories:
            self.entries.append(
                Entry(contest=self.contest, author=story.author, story=story)
            )
            self.entries[-1].save()
    ...
    def set_up_multiple_contest_components(self):
        ...
        for contest in self.contests:
            for story in self.stories:
                self.entries.append(
                    Entry(contest=contest, author=story.author, story=story)
                )
                self.entries[-1].save()
    ...
    def score_brawl(self):
        """reuable routine to assign scores to contest entries"""
        scores = [3, 5, 7, 11, 13]
        self.crits = Crit.objects.filter(
            entry__contest=self.contest, reviewer=self.contest.creator
        )
        for crit in self.crits:
            shuffle(scores)
            crit.score = scores.pop()
