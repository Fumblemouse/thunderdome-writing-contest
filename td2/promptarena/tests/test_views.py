...
class PromptArenaViewTest(BaseAppTestCase):
    ...
    def test_view_contest_details_view_exists(self):
        ...
        response = self.client.get(
            reverse("view contest details", kwargs={"contest_id": self.contest.pk})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_confirm_enter_contest_exists(self):
        ...
        self.contest.save()
        response = self.client.get(
            reverse("confirm enter contest", kwargs={"contest_id": self.contest.pk})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_enter_contest_new_story_view_exists(self):
        ...
        response = self.client.get(
            reverse("enter contest", kwargs={"contest_id": self.contest.pk})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_enter_contest_old_story_view_exists(self):
        ...
        self.sign_up_for_contest(self.contest, self.user)
        response = self.client.get(
            reverse("enter contest old story", kwargs={"contest_id": self.contest.pk})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_judgemode_crit_view_exists(self):
        ...
        for crit in crits:
            response = self.client.get(
                reverse("judgemode", kwargs={"crit_id": crit.pk})
            )
            self.assertEqual(response.status_code, 200)
            self.assertTemplateUsed(response, "promptarena/judgemode.html")
    ...
    def test_add_judge_view_exists(self):
        ...
        self.contest.save()
        response = self.client.get(
            reverse("add judge", kwargs={"contest_id": self.contest.pk})
        )
        self.assertEqual(response.status_code, 200)
...
class PromptArenaLoginAccessTest(BaseAppTestCase):
    ...
    def test_enter_contest_old_story_view_restricted(self):
        ...
        self.assertRedirects(
            response, "/accounts/login/?next=/1/enter-contest-old-story"
        )
...
class PromptArenaViewsWithData(BaseAppTestCase):
    ...
    def test_edit_contest_not_creator(self):
        ...
        response = self.client.post(
            reverse("edit contest", kwargs={"contest_id": self.contest.pk}),
            {
                "title": "My Contest title",
                "content": "My Contest content",
                "start_date": timezone.now(),
    ...
    def test_edit_contest_live_creator_access(self):
        ...
        self.assertRedirects(
            response, "/" + str(self.contest.pk) + "/view-contest-details"
        )
    ...
    def test_enter_contest_new_story_invalid_entry(self):
        ...
        #for i in range(1010):
        #    story_content += " word"
        story_content = "".join(" word" for _ in range(1010))
        response = self.client.post(
            reverse("enter contest", kwargs={"contest_id": self.contest.pk}),
            {
                "title": "My story title",
    ...
    def test_add_judge_add_same_judge(self):
        ...
        self.assertTemplateUsed(response, "promptarena/add-judge.html")
        self.assertContains(
            response, "This judge is already judging this contest", html=True
        )
    ...
    def test_enter_crit(self):
        ...
        self.login_testuser("djangotestuser1")
        response = self.client.post(
            reverse("judgemode", kwargs={"crit_id": self.crits[0].pk}),
            {
    ...
    def test_enter_crit_with_repeat_score(self):
        ...
        response = self.client.post(
            reverse("judgemode", kwargs={"crit_id": self.crits[1].pk}),
            {
                "content": self.content,
                "score": Crit.HI_SCORE,
                "final": False,
        ...
        messages = list(response.context["messages"])
        error_messages = [message for message in messages if "error" in message.tags]
        self.assertEqual(len(error_messages), 1)
        self.assertIn(
            "You have already used that rank in this contest", str(error_messages[0])
        )
