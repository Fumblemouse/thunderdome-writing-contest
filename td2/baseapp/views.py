...
def home(request):
    ...
    if request.user.is_authenticated:
        ...
        notices = Notice.objects.filter(privacy=Notice.PUBLIC)
    return render(request, "baseapp/home.html", {"notices": notices})
...
def create_story(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "Your story was submitted successfully! Hopefully it doesn't suck.",
            )
...
def edit_story(request, story_id=""):
    ...
    for entry in entries:
        if entry.contest.status not in [Contest.UNOPENED, Contest.CLOSED]:
            messages.error(
                request,
                "Your story is already in a contest.  It cannot be edited at this time.",
            )
    ...
    if form.is_valid():
        ...
        form.save()
        messages.success(
            request, "Your story was updated successfully! Hopefully it doesn't suck."
        )
        # redirect to a new URL:
        return redirect(
            "view story by slug", author_slug=story.author.slug, story_slug=story.slug
        )
...
def view_stories(request):
    ...
    return render(
        request, "baseapp/view-stories.html", {"stories_context": stories_context}
    )
...
def view_stories_by_author(request, author_slug=""):
    ...
    return render(
        request,
        "baseapp/view-stories.html",
        {"stories_context": stories_context, "author_context": author_context},
    )
...
def view_story_by_slug(request, author_slug="", story_slug=""):
    ...
    context = {
        "story_context": get_object_or_404(
            Story, author__slug=author_slug, slug=story_slug
        )
    }
    ...
    if not check_story_permissions(request, context["story_context"]):
        messages.error(request, "This story has been locked by the author.")
        return redirect("view stories")
    context["crit_context"] = Crit.objects.filter(entry__story=context["story_context"])
